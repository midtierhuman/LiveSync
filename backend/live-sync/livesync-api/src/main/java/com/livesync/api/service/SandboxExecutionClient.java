...
public class SandboxExecutionClient {
    ...
    public record SandboxResponse(String language, String status, boolean isSuccess, String message, String standardOutput, String standardError, Instant requestedAt, Instant completedAt) {}
    ...
    public List<ExecutionLanguageDescriptor> languages() {
        try {
            ...
            if (response.statusCode() / 100 != 2) throw new IllegalStateException("Sandbox returned HTTP " + response.statusCode());
    ...
    public SandboxResponse execute(SandboxRequest payload) {
        try {
            ...
            var response = client.send(request, HttpResponse.BodyHandlers.ofString());
            if (response.statusCode() / 100 != 2) throw new IllegalStateException("Sandbox returned HTTP " + response.statusCode());
            return json.readValue(response.body(), SandboxResponse.class);
