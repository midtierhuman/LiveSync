...
public class IdentityPasswordHasher {
    ...
    public String hash(String password) {
        try {
            byte[] salt = new byte[SALT_LENGTH]; new SecureRandom().nextBytes(salt);
    ...
    public boolean matches(String password, String encoded) {
        try {
            ...
            ByteBuffer input = ByteBuffer.wrap(bytes); input.get();
            ...
            String algorithm = switch (prf) { case 0 -> "PBKDF2WithHmacSHA1"; case 1 -> "PBKDF2WithHmacSHA256"; case 2 -> "PBKDF2WithHmacSHA512"; default -> null; };
            return algorithm != null && MessageDigest.isEqual(expected, derive(password, salt, iterations, algorithm));
        } catch (IllegalArgumentException | java.security.GeneralSecurityException exception) { return false; }
    }
