...
public class SecurityConfiguration {
    ...
    @Bean CorsConfigurationSource corsConfigurationSource(@Value("${livesync.cors.allowed-origins}") String origins) {
        var config = new CorsConfiguration();
        if (!origins.isBlank()) config.setAllowedOrigins(Arrays.stream(origins.split(",")).map(String::trim).toList());
        config.setAllowedHeaders(java.util.List.of("*")); config.setAllowedMethods(java.util.List.of("*"));
