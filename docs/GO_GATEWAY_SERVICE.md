...
4. **Zero-Trust JWT Authentication & Workspace Access Verification (`middleware/auth.go`)**:
   ...
   - Validates cryptographic signature, expiration, issuer (`JWTIssuer`), and audience (`JWTAudience`).
   - Extracts structured `UserClaims` and injects caller identity into request context (`UserClaimsContextKey`, `UserTokenContextKey`).
   - Performs zero-trust workspace and document authorization checks (`VerifyWorkspaceAccess`) against `livesync-api` before allowing filesystem synchronization (`/api/workspaces/:id/sync`), search & replace (`/api/workspaces/:id/search`), or PTY terminal session allocation.
